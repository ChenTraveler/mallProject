    ...
    <el-table :data="filterAutomobileInfs.slice((fy.currentPage-1)*fy.pageSize,fy.currentPage*fy.pageSize)"
      ...
      <el-table-column label="姓名"
        ...
        <template #default="scope">
          ...
          <el-tag v-if="scope.row.name==uname"
                  class="ml-2"
                  type="danger"
                  style="margin-left:5px">当前</el-tag>
          <el-tag v-if="scope.row.name=='admin'&&uname!='admin'"
                  class="ml-2"
                  type="warning">超级管理员</el-tag>
        </template>
      ...
      <el-table-column align="right">
        ...
        <template #header>
          <el-input v-model="search"
                    size="small"
                    suffix-icon="Search"
                    placeholder="用户搜索"><i slot="suffix"
               class="el-input__ icon el-icon-date"></i></el-input>
        </template>
        <!-- 用户相关操作 -->
        <template #default="scope">
          <el-button v-if="uname=='admin'||scope.row.name==uname"
                     size="small"
          ...
          <el-tag v-if="uname!='admin'&&scope.row.name!=uname"
                  class="ml-2"
                  type="info">无权限修改</el-tag>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页器 -->
      ...
      <template #footer>
        <el-form :model="form"
          ...
          <el-row justify="center">
            ...
            <!-- 用户信息编辑列 -->
            <el-row justify="center"
                    style="text-align:center;flex-direction:column;padding:15px;width:400px">
              <!-- 姓名编辑 -->
              ...
              <el-form-item label="电话："
                            v-if="form.name==uname||uname=='admin'"
                            prop="iphone">
                <el-input v-model="form.iphone" />
              </el-form-item>
              <!-- 密码编辑 -->
              <el-form-item label="密码："
                            v-if="useState==1||uname=='admin'"
                            prop="pass">
                <el-input class="ico"
                          style="position:relative"
                          :type="view?'text':'password'"
                          :suffix-icon="accordingTo?0:view?'View':'Hide'"
                          v-model="form.pass">
                </el-input>
                <i v-if="form.pass!=''"
                   style="position:absolute;right:0;width:25px;height:18px;cursor:pointer;"
                   @click="toView()"></i>
              </el-form-item>
              <!-- 状态编辑 -->
              <el-form-item label="权限："
                            v-if="form.name!=uname">
              ...
              <span class="dialog-footer"
                ...
                <el-button @click="dialogVisible = false"
                           style="margin-top:15px">取消</el-button>
                <el-button type="primary"
                           @click="confirm(form.id,ruleFormRef)"
...
import { computed, reactive, Ref, ref, getCurrentInstance ,watch} from 'vue'
//信息提示
